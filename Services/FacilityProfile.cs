...
public static class FacilityProfile
{
    ...
    public static Profile For(AppState s, BoardLoad load, string? trailerType)
    {
        ...
        // The game's window is NOT a site's opening hours, and reading it as one produced nonsense.
        //
        // Reported from play: a flatbed job with a delivery range of 16:04 to 22:04. Taken as opening
        // hours that says a construction site works four in the afternoon until ten at night. It does
        // not. What that range actually is, for a site, is the LATEST the load may land — the deadline
        // and nothing else. The driver gets there inside the site's own working day, which is usually
        // well BEFORE the window opens, and delivering early is fine.
        //
        // For a dock the same range means something quite different, and that is handled above: there
        // the opening is when your slot can start, because somebody booked you in.
        var h = SeededHours(s, load.DestCity, load.DestState, load.Receiver);

        return new Profile(kind, pct, Math.Round(h.OpenHour, 2), Math.Round(h.CloseHour, 2),
                           QueuePeakHours(s, load.DestCity, load.DestState, load.Receiver), true);
    }
